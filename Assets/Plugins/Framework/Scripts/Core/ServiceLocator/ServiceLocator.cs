...
namespace Framework.Core.Service
{
    ...
    public class ServiceLocator : IServiceLocator
    {
        ...
        public GameObject HostRoot { get; set; }
        ...
        private object CreateInstance(Type implType)
        {
            ...
            {
                // 尝试在场景中查找已存在的组件实例并复用（避免重复单例）
                var existing = UnityEngine.Object.FindFirstObjectByType(implType) as MonoBehaviour;
                if (existing != null)
                    return existing;

                // 确保有一个宿主 GameObject 用于承载服务组件
                if (HostRoot == null)
                {
                    // 如果未指定 HostRoot，创建一个默认的 persistent host（建议由 Bootstrap 提前设置）
                    HostRoot = new GameObject("ServiceHost");
                    UnityEngine.Object.DontDestroyOnLoad(HostRoot);
                }

                // 在 HostRoot 上 AddComponent（等同于 host.AddComponent<implType>()）
                var comp = HostRoot.AddComponent(implType) as MonoBehaviour;
                if (comp == null)
                    throw new InvalidOperationException($"Failed to AddComponent {implType.FullName} to host GameObject.");
